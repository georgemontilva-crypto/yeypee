                      ...
                      chars.map((ch: any) => {
                        ...
                        return (
                          <Link key={ch.id} to={`/characters/${ch.slug}`} className={`rounded-smcard border overflow-hidden transition-all ${done ? "border-jungle bg-jungle/5" : isSecret ? "border-gold/50 bg-black/5" : "border-borderc opacity-60"}`}>
                            <div className="flex items-center justify-center p-1 aspect-square sm:aspect-auto sm:h-40" style={{ background: isSecret ? undefined : (done ? undefined : ch.cardBgColor) }}>
                              {isSecret ? (
                                ...
                                <img src={ch.imageFront || undefined} alt={ch.name} className="h-full w-full object-contain scale-[1.28]" onError={(e) => { (e.target as HTMLImageElement).style.display = "none"; }} />
                              ) : (
